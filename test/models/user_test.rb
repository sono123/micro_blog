...
class UserTest < ActiveSupport::TestCase
  ...
  test "the email address should be stored as lower-case" do
  ...
  test "should follow and unfollow a user" do
    steve = users(:steve)
    arthur = users(:arthur)
    assert_not steve.following?(arthur)
    steve.follow(arthur)
    assert steve.following?(arthur)
    assert arthur.followers.include?(steve)
    steve.unfollow(arthur)
    assert_not steve.following?(arthur)
  end
  ...
  test "feed should have the right posts" do
    ...
    arthur.microposts.each do |post_unfollowed|
